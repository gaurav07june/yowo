...
public abstract class NetworkSilentActivity extends BaseActivity {
        ...
        new InternetCheck(this).isInternetConnectionAvailable(new InternetCheck.InternetCheckListener() {
            ...
            public void onComplete(boolean connected) {
                if (connected)
                    ...
                    new InternetRetryDialog(NetworkSilentActivity.this, new InternetRetryDialog.RetryInternetListener() {
                        @Override
                        public void onRetry() {
                        ...
                        public void onExit() {
                            //progressDialog.show();
                            ActivityCompat.finishAffinity(NetworkSilentActivity.this);
                        }
                    }).show();
                }
            }
    ...
    public class APICallback<T> implements Callback<GenericResponseModel<T>>
    {
        ...
        public void onResponse(Call<GenericResponseModel<T>> call, Response<GenericResponseModel<T>> response) {
            ...
            if (response.code() == Constants.API.TOKEN_EXPIRED_CODE){
                AppGlobalData.getInstance().logOut(NetworkSilentActivity.this);
                startActivity(new Intent(NetworkSilentActivity.this, SignInActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
                Toast.makeText(NetworkSilentActivity.this, "Your session has expired.", Toast.LENGTH_SHORT).show();
                finish();
